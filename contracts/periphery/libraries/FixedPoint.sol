...
library FixedPoint {
        ...
        returns (uq112x112 memory)
    {
        ...
        require(sum <= type(uint112).max, "FixedPoint::muluq: sum overflow");
        ...
        returns (uq112x112 memory)
    {
        ...
        if (self._x <= type(uint112).max) {
            uint256 value = (uint256(self._x) << RESOLUTION) / other._x;
            require(value <= type(uint112).max, "FixedPoint::divuq: overflow");
            return uq112x112(uint224(value));
        }

        uint256 result = FullMath.mulDiv(Q112, self._x, other._x);
        require(result <= type(uint112).max, "FixedPoint::divuq: overflow");
        ...
        returns (uq112x112 memory)
    {
        ...
        if (numerator <= type(uint112).max) {
            uint256 result = (numerator << RESOLUTION) / denominator;
            require(
                result <= type(uint112).max,
